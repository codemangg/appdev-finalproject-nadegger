...
```{r}
# reading the CSV file (api call was not possible)
craters <- read_csv("lunar_crater_database_robbins_2018.csv", show_col_types = FALSE)

# divide the longitudes to have near- and farside, then categorize the craters
craters <- craters %>%
  mutate(LON_CIRC_IMG = if_else(LON_CIRC_IMG > 180, LON_CIRC_IMG - 360, LON_CIRC_IMG)) %>%
  mutate(lunar_side = case_when(
    LON_CIRC_IMG >= -90 & LON_CIRC_IMG <= 90 ~ "Nearside",
    TRUE ~ "Farside"))

# create the dataframe as spatial object
craters_sf <- st_as_sf(craters, coords = c("LON_CIRC_IMG", "LAT_CIRC_IMG"), crs = 4326, remove = FALSE)
```
